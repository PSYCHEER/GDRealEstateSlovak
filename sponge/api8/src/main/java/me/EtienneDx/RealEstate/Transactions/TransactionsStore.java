...
public class TransactionsStore
{
    ...
    {
    	loadData();
    	Sponge.server().scheduler().submit(Task.builder().plugin(RealEstate.instance.pluginContainer).delay(60, TimeUnit.SECONDS).execute(
    	new Runnable()
    	{
		...
		}).interval(60, TimeUnit.SECONDS).build());
    }
	...
	public void sell(Claim claim, ServerPlayer player, double price, ServerLocation sign)
	{
		...
		RealEstate.instance.addLogEntry("[" + this.dateFormat.format(this.date) + "] " + (player == null ? "The Server" : player.name()) + 
				" has made " + (claim.isAdminClaim() ? "an admin" : "a") + " " + (claim.getParent() == null ? "claim" : "subclaim") + " for sale at " +
                "[" + Utils.getWorldName(world) + ", " +
			...
			for(ServerPlayer p : Sponge.server().onlinePlayers())
			{
				if(p != player)
				{
					Messages.sendMessage(p, RealEstate.instance.messages.msgInfoClaimCreatedSellBroadcast,
							player == null ? RealEstate.instance.messages.keywordTheServer : player.name(),
							claimPrefix,
							claimTypeDisplay,
							Utils.formatPrice(price));
				}
	...
	public void rent(Claim claim, ServerPlayer player, double price, ServerLocation sign, int duration, int rentPeriods, boolean buildTrust)
	{
	...
	public void lease(Claim claim, ServerPlayer player, double price, ServerLocation sign, int frequency, int paymentsCount)
	{
	...
	public Transaction getTransaction(ServerPlayer player)
	{
		if(player == null) return null;
		final Claim c = GriefDefender.getCore().getClaimAt(player.serverLocation());
