...
{
	public ClaimSell(Claim claim, Player player, double price, Location<World> sign)
	{
	...
	public boolean update()
	{
	    if(Utils.isBlockSign(sign.getBlockType()))
		{
			...
			if(RealEstate.instance.config.cfgUseCurrencySymbol)
			{
				...
				{
					signData.addElement(3, LegacyHexSerializer.toText(RealEstate.instance.config.cfgCurrencySymbol + " " + (int)Math.round(price)));
				}
				else
				{
					signData.addElement(3, LegacyHexSerializer.toText(RealEstate.instance.config.cfgCurrencySymbol + " " + price));
				}
			}
			else
			{
				if(RealEstate.instance.config.cfgUseDecimalCurrency == false)
				{
					signData.addElement(3, LegacyHexSerializer.toText((int)Math.round(price) + " " + RealEstate.instance.config.currencyNamePlural));
				}
				else
				{
					signData.addElement(3, LegacyHexSerializer.toText(price + " " + RealEstate.instance.config.currencyNamePlural));
				}
			}
	...
	public void interact(Player player)
	{
		...
		if(claimType.equalsIgnoreCase("claim") && !RealEstate.instance.config.cfgTransferClaimBlocks && 
				GriefDefender.getCore().getPlayerData(player.getWorld().getUniqueId(), player.getUniqueId()).getRemainingClaimBlocks() < claim.getArea())
		{
			int remaining = GriefDefender.getCore().getPlayerData(player.getWorld().getUniqueId(), player.getUniqueId()).getRemainingClaimBlocks();
		...
		if(Utils.makePayment(owner, player.getUniqueId(), price, false, true))// if payment succeed
		{
			...
			if(claim.getParent() != null || claim.getOwnerUniqueId().equals(player.getUniqueId()))
			{
                ...
                if(RealEstate.instance.config.cfgMessageOwner && owner != null)
                {
                	...
                	{
						Messages.sendMessage(oldOwner.getPlayer().get(), RealEstate.instance.messages.msgInfoClaimOwnerSold,
								player.getName(),
								claimTypeDisplay,
								Utils.formatPrice(price),
								location);
